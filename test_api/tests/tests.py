...
#                              [
#                                  (1, "New Doggie Name", "available", HttpErrorCodes.Ok),
#                                  (2, "New Kitty Name", "pending", HttpErrorCodes.Ok),
#                                  (3, "New Fishy Name", "sold", HttpErrorCodes.NotFound),
#                              ]
#                              )
...
#         """
#         This function tests the Update Pet with Form API endpoint
...
#
#         with allure.step("Check status code"):
#             assert response.status_code == expected_status_code, f"Ожидался статус-код {expected_status_code}, а получено {response.status_code}"
#
#         if expected_status_code == HttpErrorCodes.Ok:
...
#                 assert get_response.json()["name"] == name, f"Имя питомца не обновлено, ожидалось '{name}', а получено '{get_response.json()['name']}'"
#
#             with allure.step("Check updated status"):
#                 assert get_response.json()["status"] == status, f"Статус питомца не обновлен, ожидалось '{status}', а получено '{get_response.json()['status']}'"
#
#     @pytest.mark.parametrize("pet_id, api_key, expected_status_code",
#                              [
#                                  (1, "special-key", HttpErrorCodes.Ok),  # Удаление питомца с существующим ID
#                                  (2, "special-key", HttpErrorCodes.Ok),  # Удаление питомца с существующим ID
#                                  (3, "special-key", HttpErrorCodes.NotFound),  # Удаление несуществующего питомца
#                              ]
#                              )
...
#         """
#         This function tests the Delete Pet API endpoint
#         :param pet_id: ID of the pet to delete
#         :param api_key: API key for authorization (required for deletion)
#         :param expected_status_code: Expected status code of the response
#         :return:
#         """
...
#                 url=f"{base_url}/pet/{pet_id}",
#                 headers={"api_key": api_key}  # Указываем API ключ в заголовках
#             )
#
#         with allure.step("Check status code"):
#             assert response.status_code == expected_status_code, f"Ожидался статус-код {expected_status_code}, а получено {response.status_code}"
#
#         if expected_status_code == HttpErrorCodes.Ok:
#             with allure.step("Check if pet still exists"):
#                 try:
#                     get_response = requests.get(
#                         url=f"{base_url}/pet/{pet_id}"
#                     )
#                     assert get_response.status_code == HttpErrorCodes.NotFound, f"Питомец с ID {pet_id} все еще существует, ожидалось 404 Not Found"
#                 except Exception as e:
#                     print(f"Ошибка при проверке существования питомца: {e}")
#
#         print(f"Тест для ID '{pet_id}' успешно завершен.")


def test_api_framework(api_client):
    response1 = api_client.pet.get_pet_by_id(1)
    assert response1.status_code == HttpErrorCodes.Ok
